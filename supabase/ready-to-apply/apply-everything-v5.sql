...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM public.projects) THEN
    ...
    SELECT
      ...
      (5000000000.0 + ((g::bigint * 1000000000) % 5000000000))::numeric,
      ((g::bigint * 800000000) % 5000000000)::numeric,
...
-- roles / role_permissions optional — only if exist (some projects don't have them)
DO $$
BEGIN
  IF public._table_exists('roles') THEN
    GRANT SELECT ON public.roles TO anon;
  END IF;
  IF public._table_exists('role_permissions') THEN
