...
//
// Optional query param: ?tool=<name>
//   name ∈ {web_search, youtube_trending, billboard_hot_100, all}
//   When omitted, runs all tools in series.
//   When specified, returns early after that single tool completes
//   (avoids 8s+ timeout when caller only cares about one).

import { NextResponse } from 'next/server'
import { web_search, youtube_trending, billboard_hot_100 } from '@/lib/ai/tools'

const ALLOWED_TOOLS = new Set(['web_search', 'youtube_trending', 'billboard_hot_100'])

export async function GET(request: Request) {
  const url = new URL(request.url)
  const toolParam = url.searchParams.get('tool') ?? 'all'

  // Defensive early-return: invalid toolName → 400 immediately.
  if (toolParam !== 'all' && !ALLOWED_TOOLS.has(toolParam)) {
    return NextResponse.json(
      {
        ok: false,
        error: `Invalid tool: ${toolParam}`,
        allowed: [...ALLOWED_TOOLS, 'all'],
      },
      { status: 400 }
    )
  }
  ...
  // 1. web_search
  if (toolParam === 'all' || toolParam === 'web_search') {
    try {
    ...
    if (toolParam === 'web_search') {
      return NextResponse.json({ timestamp: new Date().toISOString(), total_ms: Date.now() - t0, tool: toolParam, jina_configured: !!process.env.JINA_API_KEY, results })
    }
  }

  // 2. youtube_trending
  if (toolParam === 'all' || toolParam === 'youtube_trending') {
    try {
    ...
    if (toolParam === 'youtube_trending') {
      return NextResponse.json({ timestamp: new Date().toISOString(), total_ms: Date.now() - t0, tool: toolParam, jina_configured: !!process.env.JINA_API_KEY, results })
    }
  }

  // 3. billboard_hot_100
  if (toolParam === 'all' || toolParam === 'billboard_hot_100') {
    try {
