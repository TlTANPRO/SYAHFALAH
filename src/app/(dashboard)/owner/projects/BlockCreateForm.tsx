// app/(dashboard)/owner/projects/BlockCreateForm.tsx
'use client'
...
interface Project { id: string; code: string | null; name: string }

export function BlockCreateForm({ projects }: { projects: Project[] }) {
  const router = useRouter()
  const [busy, setBusy] = useState(false)
  const [msg, setMsg] = useState<{ type: 'ok' | 'err'; text: string } | null>(null)
  const [form, setForm] = useState({
    project_id: projects[0]?.id ?? '', name: '', code: '',
    total_units: '0', sort_order: '0', description: '',
  })
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (!form.project_id || !form.name.trim()) {
      setMsg({ type: 'err', text: 'Project + name wajib.' }); return
    }
    ...
    try {
      ...
      const res = await fetch('/api/projects/blocks', {
      ...
      setMsg({ type: 'ok', text: 'Block dibuat.' })
    ...
    <Card>
      <CardHeader>
        <CardTitle className="text-base flex items-center gap-2">
          <Plus className="h-4 w-4" /> Block baru
        </CardTitle>
      </CardHeader>
      <CardContent>
        <form onSubmit={submit} className="grid grid-cols-1 md:grid-cols-3 gap-3">
          <div>
            <label className="block text-xs text-[var(--color-text-tertiary)] mb-1">Project</label>
