// app/(dashboard)/owner/reports/ReportsClient.tsx
// Card-grid layout for division reports with search + pagination.
...
interface Props {
  initialData: ReportCard[]
  total: number
}

export function ReportsClient({ initialData, total: initialTotal }: Props) {
          ...
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="card">
              <div className="card-body p-4 space-y-2">
                ...
                <div className="h-3 bg-[var(--color-surface-2)]/60 rounded animate-pulse w-3/4" />
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {rows.map((r: ReportCard) => (
            <Link
              ...
              aria-label={`Buka laporan ${r.name}`}
            >
              <div className="card-body p-4">
                ...
                {r.description && (
                  <p className="text-xs text-[var(--color-text-secondary)] line-clamp-2">{r.description}</p>
                )}
