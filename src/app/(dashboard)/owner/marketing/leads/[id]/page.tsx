// owner/marketing/leads/[id]/page.tsx
// Lead detail (from owner/marketing hub). Full profile + score + timeline.
// Same content as divisi/[id]/leads/[id] but with owner/marketing nav context.

import { createClient } from '@supabase/supabase-js'
import { notFound } from 'next/navigation'
import Link from 'next/link'
import { ArrowLeft, Phone, User, Building, Calendar, Tag, TrendingUp, FileText, ClipboardCheck, Activity, Info } from 'lucide-react'
  ...
  return (
    <div className="space-y-6">
      <Breadcrumbs crumbs={[
        ...
        { label: 'Leads', href: '/owner/marketing?tab=leads' },
        { label: 'Detail' }
      ]} />

      <HeroSection
        eyebrow={<><Info className="inline h-3 w-3 mr-1" aria-hidden /> Lead Detail</>}
        title={<h1 className="display-lg">{lead.customer_name}</h1>}
        subtitle={
          <span className="flex items-center gap-2 flex-wrap">
            ...
            <span>·</span>
            <Link href="/owner/marketing?tab=leads" className="inline-flex items-center gap-1 text-[var(--color-brand-500)] hover:underline">
              <ArrowLeft className="h-3 w-3" aria-hidden /> Kembali ke daftar
            </Link>
          </span>
        }
      />
            ...
            <div className="text-center">
              ...
              <div className="mt-4 w-full bg-[var(--color-surface-2)] rounded-full h-2 overflow-hidden">
                <div
                  ...
                  aria-label={`Score ${lead.score ?? 0} dari 100`}
                />
              </div>
      ...
      <PageHeader title="Aktivitas Terkait" subtitle="Survei, booking, SP3K, dan akad yang terkait dengan lead ini." />
