// app/(dashboard)/kepala-kantor/divisi/legal/page.tsx
// Legal / Compliance Division Detail Dashboard
...
import { Shield, Target, TrendingUp, CheckCircle, AlertTriangle, Calendar, FileText, Gavel, Scale } from 'lucide-react'
import { formatCurrency, formatPercent } from '@/lib/utils'
import { createClient } from '@/lib/supabase/client'
import { useQuery } from '@tanstack/react-query'

export default function LegalDivisionDashboard() {
  const supabase = createClient()

  const { data: divisionKPIs } = useQuery({
    queryKey: ['kpis', { division: 'LGL', level: 'division' }],
    queryFn: async () => {
  ...
  const { data: taskSummary } = useQuery({
    queryKey: ['division-task-summary', 'legal'],
    queryFn: async () => {
      const { data, error } = await supabase
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <SectionLabel number={0} title="Legal / Compliance Division" subtitle="Legal affairs, contracts & compliance" />
        </div>
        <span className="px-3 py-1 bg-purple-500/10 text-purple-500 text-sm rounded-full font-medium">LGL</span>
      </div>

      <SectionLabel number={1} title="Division KPIs (Level 3)" subtitle="Legal-specific strategic targets" />
      <BentoGrid columns={4}>
        {divisionKPIs?.map((kpi) => (
          <KPICard
            ...
            icon={
              kpi.code?.includes('SHM') ? <FileText className="h-5 w-5" /> :
              kpi.code?.includes('CONTRACT') ? <Gavel className="h-5 w-5" /> :
              kpi.code?.includes('COMPLY') ? <Scale className="h-5 w-5" /> :
              kpi.code?.includes('DOC') ? <FileText className="h-5 w-5" /> :
      ...
      <SectionLabel number={2} title="Team Personal KPIs (Level 4)" subtitle="Individual legal team performance" />
      <TableCard
        title="Legal Team KPI Status"
      ...
      <SectionLabel number={3} title="Today's Task Completion" subtitle="Legal division task progress" />
      <BentoGrid columns={2}>
        {taskSummary?.map((div) => (
          <ChartCard
            key={div.division_id}
      ...
      <SectionLabel number={4} title="Scope of Work" subtitle="Active SOWs in Legal division" />
      <BentoGrid columns={3}>
        {sows?.map((sow) => (
