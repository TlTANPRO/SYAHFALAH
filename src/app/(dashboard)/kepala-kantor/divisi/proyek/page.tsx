// app/(dashboard)/kepala-kantor/divisi/proyek/page.tsx
// Proyek & Konstruksi Division Detail Dashboard
...
import { Building2, Target, TrendingUp, CheckCircle, AlertTriangle, Calendar, Hammer, HardHat, Wrench } from 'lucide-react'
import { formatCurrency, formatPercent } from '@/lib/utils'
import { createClient } from '@/lib/supabase/client'
import { useQuery } from '@tanstack/react-query'

export default function ProyekDivisionDashboard() {
  const supabase = createClient()

  const { data: divisionKPIs } = useQuery({
    queryKey: ['kpis', { division: 'PRJ', level: 'division' }],
    queryFn: async () => {
      const { data, error } = await supabase
        .from('kpis')
        .select('*')
        .eq('division_id', '55555555-5555-5555-5555-555555555555')
        .eq('level', 'division')
  ...
  const { data: taskSummary } = useQuery({
    queryKey: ['division-task-summary', 'proyek'],
    queryFn: async () => {
      const { data, error } = await supabase
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <SectionLabel number={0} title="Proyek & Konstruksi Division" subtitle="Project execution, construction & quality control" />
        </div>
        <span className="px-3 py-1 bg-amber-500/10 text-amber-500 text-sm rounded-full font-medium">PRJ</span>
      </div>

      <SectionLabel number={1} title="Division KPIs (Level 3)" subtitle="Project-specific strategic targets" />
      <BentoGrid columns={4}>
        {divisionKPIs?.map((kpi) => (
          <KPICard
            ...
            icon={
              kpi.code?.includes('ON_TIME') ? <Calendar className="h-5 w-5" /> :
              kpi.code?.includes('QC') ? <CheckCircle className="h-5 w-5" /> :
              kpi.code?.includes('BUDGET') ? <Wrench className="h-5 w-5" /> :
              kpi.code?.includes('REVENUE') ? <TrendingUp className="h-5 w-5" /> :
      ...
      <SectionLabel number={2} title="Team Personal KPIs (Level 4)" subtitle="Individual project team performance" />
      <TableCard
        title="Proyek Team KPI Status"
      ...
      <SectionLabel number={3} title="Today's Task Completion" subtitle="Proyek division task progress" />
      <BentoGrid columns={2}>
        {taskSummary?.map((div) => (
          <ChartCard
            key={div.division_id}
      ...
      <SectionLabel number={4} title="Scope of Work" subtitle="Active SOWs in Proyek division" />
      <BentoGrid columns={3}>
        {sows?.map((sow) => (
